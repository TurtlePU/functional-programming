...
import DynFlags (GhcNameVersion)
...
data IpAddr = IPv4 Int Int Int Int | IPv6 Int Int Int Int Int Int Int Int

instance Show IpAddr where
  show :: IpAddr -> String
  show (IPv4 x y z w) = show x ++ "." ++ show y ++ "." ++ show z ++ "." ++ show w
  show (IPv6 0 0 0 0 e f g h) = "::::" ++ showHex e "" ++ ":" ++ showHex g ""

data GamerInfo = Info { gRank :: Int, gName :: String }

example :: GamerInfo
example = Info 0 "lol"

example' :: GamerInfo
example' = Info { gRank = -1, gName = "loh" }

accept :: GamerInfo -> Int
accept (Info rank name) = rank

accept' :: GamerInfo -> String
accept' Info { gRank = _, gName = name } = name
